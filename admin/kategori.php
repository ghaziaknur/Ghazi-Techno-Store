...
<br>
  ...
  while($rk = mysqli_fetch_array($sqlk)){
	...
	echo "<br><a href='?p=kategoriedit&id=$rk[idkat]'><button type='button' class='btn btn-add'>Ubah Kategori</button></a>
			<a href='?p=kategoridel&id=$rk[idkat]'><button type='button' class='btn btn-add'>Hapus Kategori</button></a>";
	echo "</div>";
	echo "</div><br>";
	echo "</div>";
  }
