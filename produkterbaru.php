...
if(empty($halaman)){
  ...
  while($rp = mysqli_fetch_array($sqlp)){
	...
	echo "<br><a href='?p=produkdetail&idp=$rp[idproduk]'><button type='button' class='btn btn-add'>Lihat Detail</button></a>
...
$sqlhal = mysqli_query($kon, "select * from produk $q order by tglproduk desc");
...
if($halaman > 1){
	$previous=$halaman-1;
	echo "<span class='kotak'><a href=?p=produkterbaru&idk=$_GET[idk]&pg=$previous>&laquo; Prev</a></span> ";
}
else
{ 
...
if($halaman < $jmlhal){
	$next=$halaman+1;
	echo "<span class='kotak'><a href=?p=produkterbaru&idk=$_GET[idk]&pg=$next>Next &raquo;</a></span>";
}
else{ 
