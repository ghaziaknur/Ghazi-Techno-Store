...
while($rc = mysqli_fetch_array($sqlc)){
  ...
  echo "<td>
  			<big>$rp[nama]</big><input type='hidden' name='id[$no]' value='$rc[idcart]'><p>
			$diskon $hargalama<br>
  ...
  echo "<a href='?p=keranjangdel&idc=$rc[idcart]&idag=$rag[idanggota]'><button type='button' class='btn-x'>X</button></a>";
  echo "</div>";
  echo "</div>";
  echo "<br>";
  echo "</div>";
...
<a href='?p=selesaibelanja'><button type='button' class='btn btn-add'>Selesai Belanja &raquo;</button></a>
</div><p>";
echo "</div>";
echo "</form>";
}else{
